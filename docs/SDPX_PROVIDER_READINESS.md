...
| Symmetric-indefinite KKT (`src/kkt.jl` dense KKT paths) | `ldlt!`, `ldlt_plan`, `ldiv!`, `solve` | `MFWorkspace` reuses pivots, blocks, weighted panel, GEMM buffers | 1x1/2x2 counts, inertia, block quality/growth, failure location | Dense primitive complete. SDPX must choose formulation, LDLT route, regularization, and fallback. |
